...
fst' :: Associate "_1" ty xs => Exp ('TyDict' xs) -> Exp ty
fst' e = Lookup' #_1 e

snd' :: Associate "_2" ty xs => Exp ('TyDict' xs) -> Exp ty
snd' e = Lookup' #_2 e

trd' :: Associate "_3" ty xs => Exp ('TyDict' xs) -> Exp ty
trd' e = Lookup' #_3 e

tuple2 :: Exp ty1 -> Exp ty2 -> Exp (Tuple2 ty1 ty2)
tuple2 e1 e2 = TypedDict
             $ #_1 @= e1
            <! #_2 @= e2
            <! nil

tuple3 :: Exp ty1 -> Exp ty2 -> Exp ty3 -> Exp (Tuple3 ty1 ty2 ty3)
...
unTuple2 :: Exp (Tuple2 ty1 ty2) -> (Exp ty1, Exp ty2)
unTuple2 e = (fst' e, snd' e)

unTuple3 :: Exp (Tuple3 ty1 ty2 ty3) -> (Exp ty1, Exp ty2, Exp ty3)
...
main :: IO ()
main = do
  print $ runGroningen fib
  -- [fizzbuzz]
  --print $ map unVString $ take 30 $ eval $ runGroningen fizzbuzz
  -- [array]
  --print $ map (V.map unVInt . unVArray) $ take 10 $ eval $ runGroningen arrayExample
